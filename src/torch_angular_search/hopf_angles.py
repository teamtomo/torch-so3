...
def get_uniform_euler_angles(
        ...
        phi_ranges: torch.Tensor = torch.tensor([[-180, 180]], dtype=torch.float64),
        theta_ranges: torch.Tensor = torch.tensor([[0, 180]], dtype=torch.float64),
        psi_ranges: torch.Tensor = torch.tensor([[-180, 180]], dtype=torch.float64),
) -> list:
    """Generate multiple sets of uniform Euler angles (ZYZ convention) using Hopf fibration.
    ...
    Returns:
        list of torch.Tensor of shape (N, 3), where list length is the number of range sets,
    ...
    out_of_plane_step = torch.deg2rad(torch.tensor(out_of_plane_step, dtype=torch.float64))
    ...
    for i in range(phi_ranges.shape[0]):
        ...
        theta_all = torch.linspace(theta_range[0], theta_range[1], n_theta, dtype=torch.float64)
        ...
        euler_angles = []
        phi_step_all = torch.clamp(torch.abs(out_of_plane_step / torch.sin(theta_all)), max=phi_max_step)
        ...
        for j, phi_step in enumerate(phi_step_all):
            ...
            phi_array = torch.arange(phi_range[0], phi_range[1], phi_step, dtype=torch.float64)
            # Create all combinations using meshgrid
            grid1, grid2, grid3 = torch.meshgrid(phi_array, this_theta, psi_all, indexing='ij')
